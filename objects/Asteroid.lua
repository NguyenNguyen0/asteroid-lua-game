...
function Asteroids(x, y, ast_size, level, sfx)
    ...
    local ASTEROID_SIZE = Global.ASTEROID_SIZE
    ...
    return {
        x = x,
        y = y,
        x_vel = math.random() * ASTEROID_SPEED * velocity,
        y_vel = math.random() * ASTEROID_SPEED * velocity,
        ...
        destroy = function(self, asteroids, index, game)
            local MIN_ASTEROID_SIZE = math.ceil(ASTEROID_SIZE / 8)

            if self.radius > MIN_ASTEROID_SIZE then
                table.insert(asteroids, Asteroids(self.x, self.y, self.radius, game.level, sfx))
                table.insert(asteroids, Asteroids(self.x, self.y, self.radius, game.level, sfx))
                table.insert(asteroids, Asteroids(self.x, self.y, self.radius, game.level, sfx))
            end
